...
> **Note**: The `ft_sensor_set_zero()` interface is not recommended. This interface uses the current force torque sensor values for load compensation, which becomes invalid when the arm's posture changes. It is recommended to use `ft_sensor_iden_load()` and `ft_sensor_cali_load()` for load identification and zeroing.
